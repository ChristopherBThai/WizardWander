...
public class Skeleton7 extends Skeleton{ //22x64 - 34 x 64
	public Skeleton7(int x, int y, int health) {
		super(x, y, health);
		this.setType(7);
		this.setSize(22, 41);
	}
	...
	public void render(Graphics win) {
		if(this.getFace()==0)
			...
			win.drawImage(sprite.getSkeleton()[type][this.getFace()], (int)getEntity().getX()-12,(int)getEntity().getY()-(62-41), null);
		healthRender(win);
	...
	public void renderDamaged(Graphics g) {
		if(damageFrame>0){
			...
			if(this.getFace()==0)
				...
				g.drawImage(sprite.damaged(sprite.getSkeleton()[this.type][this.getFace()]), (int)getEntity().getX()-12,(int)getEntity().getY()-(62-41), null);
