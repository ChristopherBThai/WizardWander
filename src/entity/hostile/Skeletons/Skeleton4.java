...
public class Skeleton4 extends Skeleton{//34x61 - 20x61
	WeaponItem weapon;
	
	public Skeleton4(int x, int y, int health) {
		super(x, y, health);
		this.setType(4);
		this.setSize(20, 41);
	...
	public void render(Graphics win) {
		if(this.getFace()==0)
			...
			win.drawImage(sprite.getSkeleton()[type][this.getFace()], (int)getEntity().getX()-14,(int)getEntity().getY()-(62-41), null);
		healthRender(win);
	...
	public void renderDamaged(Graphics g) {
		if(damageFrame>0){
			...
			if(this.getFace()==0)
				...
				g.drawImage(sprite.damaged(sprite.getSkeleton()[this.type][this.getFace()]), (int)getEntity().getX()-14,(int)getEntity().getY()-(62-41), null);
